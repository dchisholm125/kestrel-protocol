...
async function main(): Promise<void> {
  ...
  log.info('[ScenarioC] Testing Hot Key Settler Separation');
  const hotKeySettler = Keypair.generate();
  log.info(`[ScenarioC] Rotating settler to hot key: ${hotKeySettler.publicKey.toBase58()}`);
  await kestrelClient.updateSettler(hotKeySettler.publicKey);

  const buyerScenarioC = await ensureUnusedBuyerWallet('DEVNET_BUYER3_PRIVATE_KEY');
  const premiumLamportsC = Math.ceil(computePremiumLamports(SWAP_SIZE_USD_CENTS_FALLBACK, market.premiumBps, solPriceUsd) * 1.1);
  await ensureBuyerFunding(buyerScenarioC, premiumLamportsC);

  const issueResultC = await issueScenarioPolicy(
    buyerScenarioC,
    5, // Force claim path
    SWAP_SIZE_USD_CENTS_FALLBACK,
    premiumLamportsC,
  ...
  const observationC = await observeBuyerSwap(connection, buyerScenarioC);
  const payoutLamportsC = computeClaimPayoutLamports(
    observationC.actualSlippageBps,
    5,
    SWAP_SIZE_USD_CENTS_FALLBACK / 100,
    solPriceUsd,
  );

  log.info(`[ScenarioC] Settling with HOT KEY ${hotKeySettler.publicKey.toBase58()}`);
  await kestrelClient.settlePolicy({
    policyAddress: issueResultC.policyAddress,
    actualSlippageBps: observationC.actualSlippageBps,
    buyerPublicKey: buyerScenarioC.publicKey,
    buyerKeypair: buyerScenarioC,
    payoutLamports: payoutLamportsC,
  }, hotKeySettler);

  const summaryC = {
    name: 'ScenarioC (HotKey)',
    premiumLamports: premiumLamportsC,
    premiumUsd: premiumLamportsC / 1e9 * solPriceUsd,
    actualSlippageBps: observationC.actualSlippageBps,
    guaranteedSlippageBps: 5,
    payoutLamports: payoutLamportsC,
    payoutUsd: payoutLamportsC / 1e9 * solPriceUsd,
    vaultNetLamports: premiumLamportsC - payoutLamportsC,
    note: `Settled by hot key ${hotKeySettler.publicKey.toBase58()}`,
  };
  ...
  console.log(`Buyer address (Scenario C): ${buyerScenarioC.publicKey.toBase58()}`);
  console.log('');
  ...
  console.log('');
  console.log('SCENARIO C (Hot Key Settlement):');
  console.log(` Premium paid: ${formatLamports(summaryC.premiumLamports)} lamports (${formatUsd(summaryC.premiumUsd)})`);
  console.log(` Actual slippage: ${summaryC.actualSlippageBps}bps`);
  console.log(` Payout: ${formatLamports(summaryC.payoutLamports)} lamports (${formatUsd(summaryC.payoutUsd)})`);
  console.log(` Note: ${summaryC.note}`);
  console.log('');
