...
const IDL: anchor.Idl = {
  ...
  instructions: [
    ...
    {
      name: 'update_settler', discriminator: [159, 114, 242, 60, 226, 13, 201, 163],
      accounts: [
        { name: 'vault', writable: true },
        { name: 'authority', signer: true },
      ],
      args: [{ name: 'new_settler', type: 'pubkey' }],
    },
    ],
    types: [],
    };
...
export class KestrelClient {
  ...
  async initialize(settlerAuthority?: PublicKey): Promise<string> {
    const settler = settlerAuthority ?? this.wallet.publicKey;
    const tx = await this.program.methods.initialize(settler).accounts({
  ...
  async settlePolicy(params: SettlePolicyParams, settlerKeypair?: Keypair): Promise<string> {
    const settler = settlerKeypair ? settlerKeypair.publicKey : this.wallet.publicKey;
    const signers = [];
    if (params.buyerKeypair) signers.push(params.buyerKeypair);
    if (settlerKeypair) signers.push(settlerKeypair);

    const tx = await this.program.methods.settlePolicy(params.actualSlippageBps, new BN(params.payoutLamports ?? 0)).accounts({
      vault: this.findVaultPda(), policy: params.policyAddress, buyer: params.buyerPublicKey, settler, systemProgram: anchor.web3.SystemProgram.programId,
    }).transaction();
    const sig = await this.provider.sendAndConfirm(tx, signers, { commitment: 'confirmed' });
  ...
  async updateSettler(newSettler: PublicKey): Promise<string> {
    const tx = await this.program.methods.updateSettler(newSettler).accounts({
      vault: this.findVaultPda(), authority: this.wallet.publicKey,
    }).transaction();
    const sig = await this.provider.sendAndConfirm(tx, [], { commitment: 'confirmed' });
    log.signal(`[Kestrel] Settler updated to ${newSettler.toBase58()}`);
    return sig;
  }
  ...
  async getVault(): Promise<GuaranteeVault | null> {
    ...
    if (!accountInfo || accountInfo.data.length < 97) return null;
    const data = accountInfo.data;
    return {
      authority: new PublicKey(data.subarray(8, 40)),
      settler_authority: new PublicKey(data.subarray(40, 72)),
      total_premiums_collected: new BN(data.readBigUInt64LE(72).toString()),
      total_payouts: new BN(data.readBigUInt64LE(80).toString()),
      total_active_exposure: new BN(data.readBigUInt64LE(88).toString()),
  ...
  async getWalletRentExemptMinimum(): Promise<number> { return this.connection.getMinimumBalanceForRentExemption(0); }
